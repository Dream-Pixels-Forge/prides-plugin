...
If the Review phase fails and must be rolled back:

1. **Dismiss review findings**: Clear the review report and issue list
2. **Revert code changes**: If review triggered code changes, `git stash` or `git checkout` them
3. **Remove analysis artifacts**: Delete review notes, analysis files generated during this phase
4. **Report to coordinator**: State what was rolled back and why

Review artifacts are documentation — compensation is file cleanup.
